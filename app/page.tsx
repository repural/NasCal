...
type HistoricalResult = {
  ...
  indexLevels?: Record<string, { priorClose?: {date:string;value:number}|null; beforeRelease?: {close:number;minuteET:string}|null; at15?: {close:number;minuteET:string}|null; at60?: {close:number;minuteET:string}|null; dayClose?: {date:string;value:number}|null; intradayStatus?:string; releaseLabel?:string; releaseTimeET?:string; sourceUrl?:string }>;
  indexWindows?: Array<{label:string;releaseTimeET:string;nasdaq:{at15:{close:number}|null;at60:{close:number}|null};sox:{at15:{close:number}|null;at60:{close:number}|null}}>;
};
...
function archiveAnalysis(all:HistoricalResult[]){
  const verified=all.filter(r=>r.status==="verified");
  const lines=[`${verified.length} verified event outcomes in the historical archive.`];
  for(const [name,label] of [["nasdaq","Nasdaq Composite"],["sox","SOX"]]){
    const sessions=verified.flatMap(r=>{const p=r.indexLevels?.[name]?.priorClose?.value,c=r.indexLevels?.[name]?.dayClose?.value;return p&&c?[pct(c,p)]:[];});
    const windows=verified.flatMap(r=>{const x=r.indexLevels?.[name],p=x?.beforeRelease?.close,c=x?.at15?.close;return p&&c?[pct(c,p)]:[];});
    lines.push(`${label}: ${sessions.length} recorded sessions; ${sessions.filter(v=>v>0).length} closed higher, with a median previous-close to event-close move of ${sessions.length?signed(median(sessions)):"unavailable"}.`);
    lines.push(`${label} +15 minutes: ${windows.length} exact release windows; ${windows.filter(v=>v>0).length} rose, median ${windows.length?signed(median(windows)):"unavailable"}.`);
  }
  lines.push("Overlapping releases, headlines and market-wide moves affect these figures. They describe observed sessions, not the isolated impact of an event or a forecast.");
  return lines.join("\n");
}
...
export default function Home() {
  ...
  const [globalAnalysisOpen, setGlobalAnalysisOpen] = useState(false);
  ...
  return <main>
    ...
    <section className="historical-analysis-section" id="historical-analysis" aria-label="Historical analysis">
      <button className="result-toggle" type="button" aria-expanded={globalAnalysisOpen} aria-controls="archive-analysis" onClick={()=>setGlobalAnalysisOpen(open=>!open)}>Historical analysis of {verifiedResults} verified events<span aria-hidden="true">{globalAnalysisOpen?"−":"+"}</span></button>
      {globalAnalysisOpen&&<div className="analysis-panel" id="archive-analysis"><label htmlFor="archive-analysis-text">Nasdaq and SOX archive summary</label><textarea id="archive-analysis-text" readOnly value={archiveAnalysis(archive.results as HistoricalResult[])} /></div>}
    </section>
