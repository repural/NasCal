...
```text
app/
  page.tsx                 Main calendar and historical-results interface
  globals.css              Site styling
public/
  event-results.json       Persistent historical outcome dataset
tests/                     Render and component checks
...
`public/event-results.json` is the analytical source of truth. New completed events should be appended after verification. Existing records should not be silently overwritten; corrections should retain an audit note and an updated verification date.
...
2. Update the event definition and/or `public/event-results.json`.
